...
rule full:
    input:
        BW_DIR + "/{sample}{type}_mqa_cov_RPKM_75.bw",
        BW_DIR + "/{sample}{type}_mqa_cov_None_1.bw",
        BW_DIR + "/{sample}{type}_mqa_cov_zero.bed.gz",
        BOWTIE2_DIR + "/{sample}{type}.bam",
        BW_DIR + "/{sample}{type}.open.bw"
    output:
        "snake_res/{sample}{type}_full.txt"
    shell:
        "touch {output}"

rule chip:
    input:
        BW_DIR + "/{sample}{type}_cov_RPKM_75.bw",
        BW_DIR + "/{sample}{type}_cov_None_1.bw",
        BOWTIE2_DIR + "/{sample}{type}.bam"
    output:
        "snake_res/{sample}{type}_chip.txt"
    shell:
        "touch {output}"
rule simple:
    input:
...
rule bowtie2:
    ...
    run:
        ...
        shell("""
            ...
            --very-sensitive --xeq --dovetail -p {threads} --mm --no-unal 2>{log} | \
...
rule mapq:
    input:
        BOWTIE2_DIR + "/{sample}{type}.bam"
    output:
        BOWTIE2_DIR + "/{sample}{type}_mapq8.bam"
    threads:
        threads_bowtie2
    log:
        BOWTIE2_DIR + "/{sample}{type}_mapq8.log"
    run:
        shell("""
...
rule methylQA:
    input:
        BOWTIE2_DIR + "/{sample}{type}{mapq}.bam"
    output:
        METHYLQA_DIR + "/{sample}{type}{mapq}.open.tmp.bed"
    log:
        METHYLQA_DIR + "/{sample}{type}{mapq}.log"
    params:
        rootname = METHYLQA_DIR + "/{sample}{type}{mapq}"
    run:
...
rule methylQA_filter:
    input:
        METHYLQA_DIR + "/{sample}{type}{mapq}.open.tmp.bed"
    output:
         METHYLQA_DIR + "/{sample}{type}{mapq}.open.bed"
    run:
        shell("""
...
rule cutsite_bw:
    input: 
        METHYLQA_DIR + "/{sample}{type}{mapq}.open.bed"
    output: 
        BW_DIR + "/{sample}{type}{mapq}.open.bw"
    params:
        bdg = BW_DIR + "/{sample}{type}{mapq}.open.bedGraph"
    log:
        BW_DIR + "/{sample}{type}{mapq}.open.bwgen.log"
    run:
        shell("""
...
rule mqa_reads:
    input:
        METHYLQA_DIR + "/{sample}{type}{mapq}.open.bed"
    output:
        METHYLQA_DIR + "/{sample}{type}{mapq}.open.reads.txt"
    shell:
        "cut -f4 {input} > {output}"

rule bam_filter_methylQA:
    input:
        bam = BOWTIE2_DIR + "/{sample}{type}{mapq}.bam",
        reads = METHYLQA_DIR + "/{sample}{type}{mapq}.open.reads.txt"
    output:
        BOWTIE2_DIR + "/{sample}{type}{mapq}_mqa.bam"
    log:
        BOWTIE2_DIR + "/{sample}{type}{mapq}_mqa.log"
    run:
        shell("""
...
rule sambamba_mkdup:
    input:
        BOWTIE2_DIR + "/{sample}{type}{mapq}.bam"
    output:
        BOWTIE2_DIR + "/{sample}{type}{mapq}_mkdup.bam"
    threads:
        threads_bowtie2
    run:
        shell("""
...
rule sambamba_mkdup:
    input:
        BOWTIE2_DIR + "/{sample}{type}{mapq}.bam"
    output:
        BOWTIE2_DIR + "/{sample}{type}{mapq}_rmdup.bam"
    threads:
        threads_bowtie2
    run:
        shell("""
            {sambamba} markdup -t {threads} -r {sambamba_options} {input} {output} 1>{log} 2>&1
        """)
        shell("""
            {samtools} index {output}
        """)
...
rule bam_coverage:
    input:
        BOWTIE2_DIR + "/{sample}{type}{mapq}{mqa}.bam"
    output:
        BW_DIR + "/{sample}{type}{mapq}{mqa}_cov_{norm}_{res}.bw"
    threads:
        threads_bowtie2
    log:
        BW_DIR + "/{sample}{type}{mapq}{mqa}_cov_{norm}_{res}.log"
    shell:
        "{bamCoverage} -b {input} -o {output} -p {threads} --normalizeUsing {wildcards.norm} -bs {wildcards.res} 1>{log} 2>&1"

rule zero_cov_regions:
    input:
        BW_DIR + "/{sample}{type}{mapq}_mqa_cov_None_1.bw"
    output:
        BW_DIR + "/{sample}{type}{mapq}_mqa_cov_zero.bed.gz"
    params:
        bed = BW_DIR + "/{sample}{type}{mapq}_mqa_cov_zero.bed"
    log:
        BW_DIR + "/{sample}{type}{mapq}_mqa_cov_zero.log"
    shell:
